...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase
    .from('field_photos')
    .insert({ field_id, url, caption, user_id: user.id })
    .select()
    .single()
...
export async function DELETE(req: Request) {
  ...
  const { data: photo } = await supabase
    .from('field_photos').select('url').eq('id', id).eq('user_id', user.id).single()
  if (photo?.url) {
    const path = photo.url.split('/field-photos/')[1]
    if (path) await supabase.storage.from('field-photos').remove([path])
  }

  const { error } = await supabase.from('field_photos').delete().eq('id', id).eq('user_id', user.id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
